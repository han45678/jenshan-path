<template>
  <!--  表單改版型過 -->
  <div id="order" class="order relative text-center bg-[#0008]">
    <div class="order-section">
      <img class="l" src="./s1/l.webp" alt="logo" data-aos="fade" data-aos-delay="200" />

      <img class="logo" src="@/section/s1/logo.svg" alt="logo" data-aos="fade-up" v-if="!isMobile" />
      <div class="order-title text-center font-['Noto_Serif_TC',serif] " v-if="info.order.title"
        v-html="info.order.title"></div>
      <div class="order-subTitle text-center text-white" v-if="info.order.subTitle"
        v-html="$isMobile() && info.order.subTitle_mo ? info.order.subTitle_mo : info.order.subTitle"></div>

      <!-- Form -->
      <div class="form mx-auto relative flex justify-center  text-white">
        <div class="left h-full flex flex-col justify-between items-center">
          ...
          <div class="gender">
            ...
            <label><input type="radio" name="gender" value="女"
                @input="(event) => (formData.gender = event.target.value)">小姐</label>
          </div>
          ...
          <div class="row room_type"><span>需求房型</span>
            <label><input type="radio" name="room_type" value="100坪"
                @input="(event) => (formData.room_type = event.target.value)">100坪</label>
            <label><input type="radio" name="room_type" value="110坪"
                @input="(event) => (formData.room_type = event.target.value)">110坪</label>
          </div>
        </div>
        <div class="right">
      ...
      <div class="flex order-send mx-auto justify-between flex-wrap flex-col relative z-50 items-center">
        ...
        <div>
        <!-- Recaptcha -->
          <vue-recaptcha class="flex justify-center mt-8 mb-8 z-10" ref="recaptcha" :sitekey="info.recaptcha_site_key_v2"
            @verify="onRecaptchaVerify" @expired="onRecaptchaUnVerify" />
          <!-- Send -->
          <div class="sendall mx-auto mt-0">
  ...
  */
  ...
  .form {
    ...
    .row {
      ...
      select {
        background: transparent url("//h35.banner.tw/img//select.svg") no-repeat calc(100% - .5em) 100%;
